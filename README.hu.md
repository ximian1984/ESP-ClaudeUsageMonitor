...
## 0/b. Milyen eszközön fut

Két lapkacsaládot támogat, **ugyanazzal a firmware-rel** — csak a kijelző-réteg más. A különbség a PlatformIO
env-ben van, forráskódot nem kell hozzányúlni.
...
Nincs közbülső szerver és nincs felhős fiók: a dongle **közvetlenül** hívja a szolgáltató API-ját, és a tokenek
végig az eszközön maradnak.

```mermaid
flowchart TD
    USB["USB-táp 5 V<br/>(csak áram, adat nem kell)"] --> ESP

    subgraph ESP["ESP32 — a kütyü"]
        direction TB
        WIFI["Wi-Fi STA<br/>20 profil, prioritással"] --> NTP["NTP óra + időzóna<br/>(a TLS-hez is kell)"]
        NTP --> AUTH["OAuth token az NVS-ből<br/>lejárat előtt magától frissül"]
        AUTH --> FETCH["HTTPS GET — usage-végpont<br/>beégetett gyökértanúsítvánnyal"]
        FETCH --> PARSE["usage_parser<br/>limits-tömb → session / heti %"]
        PARSE --> CACHE["gyorsítótár + utolsó ismert reset<br/>(NVS — újraindítást is túlél)"]
        CACHE --> LCD["LCD: %, sáv, visszaszámláló"]
        CACHE --> MIRROR["/screen — webes kijelző-tükör<br/>a valódi képpontok"]
    end

    FETCH <-->|"HTTPS"| API["api.anthropic.com<br/>(ill. OpenAI / Google / xAI)"]
    LCD --> EYE(["👁 ránézel"])
    MIRROR --> BROWSER(["böngésző a helyi hálón"])
```

**Beállítási (setup) út** — ezen adod meg a **saját** hitelesítőidet, futásidőben:

```mermaid
flowchart LR
    A["nincs ismert Wi-Fi<br/>VAGY BOOT gomb"] --> B["saját Wi-Fi-t nyit<br/>ClaudeMonitor-XXXX, WPA2<br/>a jelszó a kijelzőn"]
    B --> C["rácsatlakozol telefonnal<br/>http://192.168.4.1"]
    C --> D["Wi-Fi-profil + AI-profil<br/>megadása"]
    D --> E["OAuth: megnyitsz egy linket,<br/>jóváhagyod, a kódot bemásolod"]
    E --> F["a token az NVS-be kerül<br/>⛔ a repóba SOHA"]
```

### A hitelesítők soha nincsenek a forrásban

Ez a projekt szabálya, és a kód első sora is kimondja (`src/config.h:1` — *„Titok NEM lehet itt"*):

- A Wi-Fi-jelszót és az AI-tokeneket **te adod meg futásidőben**, a setup-felületen; az **NVS**-be kerülnek.
- A repóban **nincs** `.env`, nincs beégetett kulcs, és a `.gitignore` ki is zárja az ilyen fájlokat.
- A felület a titkokat **soha nem adja vissza**, a soros napló nem írja ki, a kijelzőre nem kerülnek.
- A forrásban szereplő `CLIENT_ID`-k **nyilvános** OAuth-kliensazonosítók (OAuth-ban a `client_id` definíció szerint
  publikus; a hivatkozott forrás fájl:sor szinten megvan a `src/config.h`-ban). Az egyetlen `CLIENT_SECRET` a Google
  **installed-app** kliense, amit a Google szabálya szerint nem kezelnek titokként, és szó szerint a nyilvános
  `gemini-cli` forrásból való — a `src/config.h:84-94` hivatkozza az eredetit.

### Flashelés: a két lap nem egyforma

⛔ A [`tools/flash.sh`](tools/flash.sh) **csak a dongle-ra** jó. A CYD más paramétereket kíván:

| | T-Dongle-S3 | ESP32-2432S028R (CYD) |
|---|---|---|
| Chip | `esp32s3` | `esp32` |
| USB–soros | beépített USB JTAG/serial (`303A:1001`) | **CH340** (külön illesztő kellhet) |
| Baud | 921 600 | **460 800** |
| Bootloader offset | **`0x0`** | **`0x1000`** |
| Flash méret | 16 MB | 4 MB |
| Partíciók | `default_16MB.csv` | `min_spiffs.csv` |
| Letöltési mód | BOOT nyomva tartás + bedugás | általában magától (DTR/RTS) |

A pontos lépések: 5. (dongle) és 2/b. (CYD).

### Webes kijelző-tükör (`/screen`)

A `http://<eszköz-IP>/screen` címen **pontosan az látszik, ami az LCD-n** — nem újrarajzolt HTML, hanem a dongle
által küldött **valódi képpontok** (a dongle-on 160×80, a CYD-n 320×240). Hibakereséshez és képernyőképhez.
...
Kijelző-pinek (hivatalos LilyGO forrás, fájl:sor a belső tervdokumentációban):
...
![A CYD állva (90°): SESSION used 16%, WEEKLY used 72%, maradék-sávokkal](docs/cyd/photo_cyd_portrait.jpg)

*Fotó, 2026-09-21 20:15: a CYD állva (90°), `esp32-2432s028r-inv`: `SESSION used 16%` (a `remaining` csík 84 %,
zöld), `WEEKLY used 72%` (a csík 28 %, sárgul).*
...
### 5/b. Feltöltés PlatformIO nélkül
...
Mérve (2026-09-17, macOS 26.6.2, `/usr/bin/python3` 3.9.6, nincs brew/pio):
...
```sh
ssh <a masik gep>                    # vagy helyben azon a gepen
cd ~/cmon-flash
...
- Végpontok/azonosító a Claude Code kliensből (forrás: a Claude Code kliens, a belső tervdokumentáció szerint). PKCE S256 + state (CSRF).

### ChatGPT, Gemini, Grok — ⚠ megírva, vason MÉG NEM futott (2026-09-17)

Ugyanaz a profil-lista és kijelző; a **Source** mezőben választható. A forrás és a mérés (hamis tokennel) a belső tervdokumentációban van. Mindhárom **nem hivatalos, belső végpont**, egy másik alkalmazás (Codex CLI / Gemini CLI / Grok CLI)
...
Nem hivatalos, nem stabil API-k. A mért tények a belső tervdokumentációban vannak.
...
| `CLOUDFLARE` / `ERROR 403` | Cloudflare-kihívás a claude.ai-on — **csak sessionKey (web) profilnál** várható; az OAuth-út hosztjai nem adnak kihívást (mérve) | sessionKey-nél: ⚠ [vason mérendő], lásd a belső tervdokumentációt; javasolt OAuth-profilra váltani |
...
## 17. Felelősség és a szolgáltatók feltételei

- A firmware **nem hivatalos, nem dokumentált** kvóta-végpontokat hív, más kliensek nyilvános OAuth-azonosítóival.
  Ezek bármikor megváltozhatnak vagy eltűnhetnek, és a használatuk **ütközhet a szolgáltató felhasználási
  feltételeivel**. Mindenki a saját felelősségére használja, a **saját** fiókjával.
- A projekt semmilyen kapcsolatban nem áll az Anthropickal, az OpenAI-jal, a Google-lel vagy az xAI-jal.
- A kijelzett értékek tájékoztató jellegűek: gyorsítótárazott adatok, és a végpont változása esetén félrevihetnek.
  A kijelző a jobb felső sarokban mindig mutatja az **adat korát** és a hibaállapotot.
